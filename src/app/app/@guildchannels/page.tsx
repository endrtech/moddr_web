...
export default function GuildActivity() {
    ...
    useEffect(() => {
        if (guildId) {
            const getData = async () => {
                const getCases = await getGuildChannels(guildId);
                setChannels(getCases);
            ...
            const useSocket = async () => {
                ...
                socket.on("newGuildChannel", (data) => {
                    setChannels((prev: any) => [...prev, data]);
                });

                socket.on("removeGuildChannel", (data) => {
                    setChannels((prev: any) => prev.filter((member: any) => member.id !== data.id));
                });
